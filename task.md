...
- [x] `cardManager.js` に新効果キーワード（✨情熱/💡発想/🚥整理/🤹並行/💤疲労）のパース追加
- [x] `gameState.js` に `tokens` フィールド追加、`placed` を配列構造に変更
- [x] `turnManager.js` でトークン消費ロジック追加
- [x] `uiController.js` で新効果アイコン表示・並行配置UI対応
...
- [x] `uiController.js` に研修リフレッシュUI追加（ボタン条件表示・残回数表示）
- [x] `gameState.js` にリフレッシュ残り回数管理追加
- [x] `saveManager.js` にリフレッシュ回数の保存復元追加
- [x] デッキ一覧画面にリフレッシュ残回数表示追加
