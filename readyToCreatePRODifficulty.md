...
### PROのスコア計算
- 各目標値（退塾/体験/入退差/内部満足の各閾値）と配点は game/rankPro.csv に記載
- "基準"以上(退塾:以下)の値を取ったら"スコア"を得る
  - (例)Dランクの体験基準は15, Dランクの動員スコアは1。体験が15以上ならば、動員目標ではスコア1を獲得
- "ランク点"の合計がそのプレイのスコアとなり、"ランク基準"以上かどうかで総合ランクを判定
  - (例)退塾3->退塾スコア-1, 体験38->動員スコア3, 満足32->内部満足スコア1, 入退差28->入退差スコア5, 合計スコア8->(ランク基準スコアを参照して)Aランク
- ランク体系: F〜SSの**11段階**
...
| 発想 | 💡 | 行動フェーズ（カード効果適用時） | **次のターン**の研修フェーズ | SRカード3枚から1枚を追加習得 | ✅ |
...
**全4ステータス共通**（3要素、プログレスバー下に折り返し表示）:
1. `C` — 現在の項目別ランク（ランクアルファベット）
2. 細いプログレスバー — `currentThreshold` を 0%、`nextThreshold` を 100% とする
3. `Aまであと8` — 目標ランク名と残り数値（プログレスバーの次の行に配置）

### プログレスバー終点ロジック（`getStatusRank(statKey, value, difficulty)`）

**FRESH 体験・入塾**: 次のランクの閾値（従来どおり）

**FRESH 満足**: Aランク未到達 → Aランク閾値（15）、A以上 → 次のランク閾値

**FRESH 経理**: Sランク未到達 → Sランク閾値（14）、S以上 → 次のランク閾値

**PRO 体験**:
1. 現在の動員スコア = rankTableを高→低で走査し `scores.mobilization !== null && thresholds.experience <= value` の最初の行のスコア
2. 次の終点 = rankTableを低→高で走査し `scores.mobilization > 現在スコア` の最初の行の `thresholds.experience`

**PRO 入塾**（退塾=0仮定）:
1. 現在の入退差スコア = rankTableを高→低で走査し `scores.enrollmentDiff !== null && enrollmentDiffThreshold <= value` の最初の行のスコア
2. 次の終点 = rankTableを低→高で走査し `scores.enrollmentDiff > 現在スコア` の最初の行の `enrollmentDiffThreshold`
...
### レイアウト
- `.status-rank-info` に `flex-wrap: wrap` を追加
- `.rank-deficit` に `width: 100%` を追加して次の行に折り返し・中央揃え
...
reset(difficultyId = 'fresh') {
    ...
    this.tokens = {
        ...
        inspiration: 0,  // 💡 発想: 次ターンの研修でSRカード3枚から1枚を追加習得×N回
...
## 11. イベントモード（FRESH・PRO共通） — ✅実装済み

期間限定イベントはタイトル画面で選択し、開始時にイベントID・名称・有効状態をゲーム状態へ固定する。FRESH、PRO、計算機モードのすべてで同じイベントアイテム処理を適用する。

### イベント用データと状態

- `eventManager.js` にイベントレジストリ、開催期間（JST）、アイテム定義を保持する。
- アイテムはカードと別管理し、デッキ・手札・配置済みカード・研修候補・削除対象には含めない。
- アイテムごとに入手状態、使用回数、ターン内回数、発動予約、解決済み数を保存する。
- イベント中のハイスコアは `difficulty × eventId` ごとに通常モードと分離する。

### 初回イベント「お盆明け、講習の前に講習！26夏期杯」

| 塾アイテム | 共通ルール |
|---|---|
| 🏆新聞取材 | 動員カテゴリの有効カードを3枚以上使ったターンに体験+2（ターン中1回）。 |
| 🏆アイデアの化学反応 | 所持カード10枚以上でSSRカード3枚から必ず1枚を獲得（ゲーム中1回）。 |
| 🏆春休みの宿題 | 3月下旬以降、教務カテゴリの有効カード3枚以上ごとに予約し、最終ターンに予約ごとに入塾+3。 |

### スコアと送信

- イベント時だけ上限超過スコアを適用する。FRESHは観点別得点8点時に体験30・入退差30超を追加加点し、PROは体験50・入退差48・満足35超を小数加点する。
- GASは `displayScore` と `points` を100まで受け付ける。イベントの送信モードは `イベント:(イベント名)`、計算機併用時は末尾に `/計算機` を付与する。
